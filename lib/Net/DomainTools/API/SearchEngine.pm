package Net::DomainTools::API::SearchEngine;
...
Net::DomainTools::API::SearchEngine - Search domain names for availability

=head1 SYNOPSIS

  use Net::DomainTools::API::SearchEngine;

  my $api = new Net::DomainTools::API::SearchEngine->new(
                key => '12345',
                partner => 'yourname',
                customer_ip => '1.2.3.4'
  );
...
sub domain_is_available {
    ...
    if ( !first { lc($_) eq lc($top_level_domain) }
        @Net::DomainTools::API::VALID_TLDS )
    {
    ...
    my $result = $self->request(
        ext =>
            $self->_tld_list_to_ext_param(@Net::DomainTools::API::VALID_TLDS),
        q          => $domain_name,
        exactfirst => 'y',
    );
